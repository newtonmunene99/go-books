...
func (cu *CategoryUpdate) AddBooks(b ...*Book) *CategoryUpdate {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
...
func (cu *CategoryUpdate) ClearBooks() *CategoryUpdate {
...
func (cu *CategoryUpdate) RemoveBooks(b ...*Book) *CategoryUpdate {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
...
func (cu *CategoryUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if cu.mutation.BooksCleared() {
		edge := &sqlgraph.EdgeSpec{
			...
			Columns: category.BooksPrimaryKey,
			Bidi:    false,
...
// AddBooks adds the "books" edges to the Book entity.
func (cuo *CategoryUpdateOne) AddBooks(b ...*Book) *CategoryUpdateOne {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
...
// ClearBooks clears all "books" edges to the Book entity.
func (cuo *CategoryUpdateOne) ClearBooks() *CategoryUpdateOne {
...
// RemoveBooks removes "books" edges to Book entities.
func (cuo *CategoryUpdateOne) RemoveBooks(b ...*Book) *CategoryUpdateOne {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
